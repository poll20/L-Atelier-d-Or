...
// import ring1 from "../video/ringcom.mp4";
...
//     const videos = videoRefs.current[0];
...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    // ✅ Just play the video freely on loop — no scroll involvement at all
    video.play().catch(() => { });
    ...
    const ctx = gsap.context(() => {
      ...
      // Master timeline — ONLY controls text, video is completely independent
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: wrapperRef.current,
          start: 'top top',
  ...
  return (
    <div ref={wrapperRef} className="bg-black" style={{ height: `calc(100vh + ${SCROLL_LENGTH}px)` }}>
      <section
        ...
        className="relative h-screen w-full overflow-hidden bg-black flex items-center justify-center pointer-events-none sm:pointer-events-auto"
      >
        {/* 🎥 Freely looping video — zero scroll coupling */}
        <div className="absolute inset-0 z-0">
          <video
            ref={videoRef}
            src={ring1}
        ...
        {/* Text Scenes — scroll driven only */}
        <div className="relative z-20 w-full max-w-7xl mx-auto px-6 h-full flex items-center justify-center">
          {SCENES.map((scene, i) => (
            <div
              key={i}
            ...
            >
              ...
              <p className="text-sm md:text-lg font-light text-[#f5f0e8]/80 max-w-xl leading-relaxed mb-12 px-4">
                {scene.sub}
              </p>
